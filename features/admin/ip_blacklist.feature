...
Feature: Admin panel can be restricted by IP blacklist
    ...
    Scenario: Per-admin blacklist denies access when the global list does not match
        Given the admin IP blacklist is enabled with global CIDRs "203.0.113.0/24"
        And administrator "admin@example.com" has per-admin IP blacklist enabled with CIDRs "127.0.0.1"
        When I am logged in as "admin@example.com" administrator
        And I open any admin page
        Then the admin response status should be 403
    ...
    Scenario: Disabled per-admin entry is ignored
        Given the admin IP blacklist is enabled with global CIDRs "203.0.113.0/24"
        And administrator "admin@example.com" has per-admin IP blacklist disabled with CIDRs "127.0.0.1"
        When I am logged in as "admin@example.com" administrator
        And I open any admin page
        Then the admin response status should be 200

    Scenario: Admin can list IP blacklist administrators
        Given the admin IP blacklist is enabled with global CIDRs "203.0.113.0/24"
        When I am logged in as "admin@example.com" administrator
        And I open the IP blacklist admins page
        Then the admin response status should be 200
        And I should see the IP blacklist admins list
