...
- **Revoke individual session** — a POST form per row marks `revoked_at` on a single record. The *current* session is intentionally non-revocable to avoid users locking themselves out by accident: the row in the UI hides its revoke button and the controller also refuses the request (HTTP 400) if the target session ID matches the request's session ID. Sign out instead.
