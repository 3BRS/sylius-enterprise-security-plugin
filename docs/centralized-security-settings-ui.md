...
- **Storage**: one row per `(path, scope)` pair, value stored as JSON. The `SettingsProvider` reads the table once per request, in-memory cached, and falls back to YAML defaults when a row is missing — so the plugin works out of the box against an empty table, and writing rows (through the `three_brs_security_settings` fixture, or by saving the form) is optional.
...
The `Linked 3rd-party OAuth accounts` shop account-menu item follows the provider registry, which reads the same `oauth.<provider>.enabled` paths the sign-in buttons do, so turning every provider off in the Settings UI hides the entry on the next request. The administrator's own page is reached from the **Security** dropdown in the header of their user edit screen rather than from the Configuration menu, and that dropdown is not gated on the toggles.
