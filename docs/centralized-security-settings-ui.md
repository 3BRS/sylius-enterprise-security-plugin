...
- **Scopes**: separate `Customers` and `Administrators` views. The storage model also defines a third, `global` scope for values that carry no scope dimension, but nothing writes to it today — the values that would live there (2FA issuer, trusted-device window, passkey relying-party data, GeoIP service ID) come from YAML instead, for the reasons given below.
