...
- **Unblock account** — sets `enabled = true`, and the customer can sign in again immediately. Offered only for an account an administrator blocked: while the customer has an open [deletion request](account-deletion-gdpr.md) the disabled flag is what enforces the scheduled erasure, so the section replaces this button with a link to the deletion overview and the action itself refuses, leaving `enabled` untouched. Cancel the request there first — cancelling re-enables the account and is recorded against the administrator who did it.
