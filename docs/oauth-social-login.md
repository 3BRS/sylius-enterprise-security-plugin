...
- **Apple works out of the box (no session tweak needed)** — Apple returns its callback as a cross-site `POST` (`response_mode=form_post`), so the browser does not send the default `SameSite=Lax` session cookie and the OAuth `state` would otherwise be lost. The plugin carries the `state` (and, for account linking, the initiating user) in a dedicated `SameSite=None; Secure; HttpOnly` single-use cookie, validates it on the callback and clears it immediately. You do **not** need to relax your application's `framework.session.cookie_samesite`. Because the cookie is `Secure`, Apple sign-in requires HTTPS (Apple mandates HTTPS redirect URIs anyway). Google and Microsoft are unaffected — they use a normal GET redirect and keep using the session.
