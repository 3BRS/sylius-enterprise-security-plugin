...
- Last-auth-method protection: `LastAuthMethodGuard` counts passkeys, social links and the password together, and the user cannot remove the last sign-in method on their account. Each one counts only while it would still open a session — a passkey while the feature is on for the group, a social link while its provider is enabled, a password while password login is on.
...
- Plugin adds a **Passkeys** entry to the shop account menu (`sylius.menu.shop.account`) automatically, shown only when the feature is enabled for customers. An administrator reaches their own list from the **Security** dropdown in the header of their user edit page, alongside two-factor authentication, social accounts and active sessions; that dropdown is rendered for the signed-in administrator's own record and the page answers 404 while the feature is off for administrators.
